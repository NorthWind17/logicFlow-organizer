<template>
  ...
  >
    ...
    <div
      class="node-option"
      v-show="
        (properties.status === 'selected' || properties.status === 'hovered') &&
        !disabled
      "
      @mousedown.capture="selectNode"
    >
      <template
        v-if="!['pageEnd', 'pageInit'].includes(properties.componentName)"
      >
        <el-tooltip
          class="item"
          effect="dark"
        ...
        >
          ...
          >
            ...
            <span class="option-icon" slot="reference" @mousedown.stop>
              <img
                src="https://s3-gzpu.didistatic.com/tiyan-base-store/suda/organizer/icons/node_abstract.png"
              />
            </span>
          </el-popover>
        </el-tooltip>
    ...
    <div class="node-next" v-if="properties.componentName != 'pageEnd'">
      <span
        v-show="
          properties.status === 'selected' || properties.status === 'hovered'
        "
...
export default {
  ...
  methods: {
    ...
    getClassName() {
      return (
        (this.model.getNodeClassName && this.model.getNodeClassName()) || ''
      )
    },
    getWarning() {
    ...
    enterNode() {
      ...
      if (this.model.properties && this.model.properties.status === 'selected')
        return
    ...
    deleteNode() {
      // .getRelatedEdgesByType(this.model.id)
      // console.log('delModel==', this.graphModel)
      // return
