<template>
  <div class="line-content" :class="isSelected && 'selected'">
    ...
    >
      <img
        class="icon"
        src="https://s3-gzpu.didistatic.com/tiyan-base-store/suda/organizer/icons/edge_add.png"
        alt=""
      />
    </div>
    <!-- <el-popover
...
export default {
  ...
  methods: {
    getConditionItem(item) {
      ...
      return this.formatValue(item.key, item)
    },
    formatValue(val, item) {
      ...
      switch (val.type) {
        case 'urlParam':
          return `${val.value}的${item.value.value}`
        case 'component':
