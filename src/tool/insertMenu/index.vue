<template>
  <div class="node-menu-wrapper">
    ...
    <!-- 连接至已有节点 -->
    <div class="node-add-title" v-if="showConnectBlock && nodeOptions.length">
      连接至已有节点:
    </div>
    <div
      style="text-align: right; margin: 0"
      v-if="showConnectBlock && nodeOptions.length"
    >
      <el-menu
        mode="vertical"
        class="node-menu"
        :open="true"
        @select="connectToNode"
      >
        <el-menu-item
        ...
        >
          <span
            @mouseenter.stop="handleNodeEnter(item.value)"
