...
class Graph {
  constructor(options) {
    ...
    this.layout = new Layout(this.lf, {
      // 布局
      rankdir: 'LR',
      align: 'UL',
      nodesep: 2,
  ...
  hoverNodesByModel(model) {
    ...
    nodes.forEach((node) => {
      ...
      if (node.properties && node.properties.componentId === componentId) {
        ...
        if (nodeModel.properties && nodeModel.properties.status === 'selected')
          return
        nodeModel.setProperties({
          status: 'normal'
        })
  ...
  hoverNode(seletedModel) {
    ...
    const nodeModel = this.lf.getNodeModelById(nodeId)
    this.hoveredNodes &&
      this.hoveredNodes.forEach((nodeModel) => {
        nodeModel.setProperties({
          status: 'normal'
        })
      })
  ...
  addNodeByDrop(model, e) {
    ...
    const x =
      Math.abs(canvasPoint.x + NODE_WIDTH / 2 - GRAPH_START_X) < 50
        ? GRAPH_START_X
        : canvasPoint.x + NODE_WIDTH / 2
  ...
  insertNode(nodeModel, type, properties) {
    ...
    if (nodeModel.type === logicLine.type) {
      ...
      const preEdge = {
        // 基于此边新建一条边，代替此边，连接新节点
        ...edge.data,
        targetNodeId: id,
        type: logicLine.type
      }
      const nextEdge = {
        // 新建一条边，连接新节点与原来此边之后的节点
        id: genId(10),
        sourceNodeId: id,
      ...
      console.log('type2', lf)
      // lf.graphModel.removeEdgeBySource(nodeModel.id)
      // 如果是基于node插入节点，则在此node之后新增一个节点
      const id = genId(10)
  ...
  deleteNode(nodeModel) {
    const incommingEdges = this.lf.getNodeIncomingEdge(nodeModel.id) //获取所有以此节点为终点的边
    const outgoingNodes = this.lf.getNodeOutgoingNode(nodeModel.id) //获取所有下一级
    const incomingNodes = this.lf.getNodeIncomingNode(nodeModel.id) //获取节点所有的上一级节点
    if (outgoingNodes.length && incommingEdges.length) {
      // 如果有后续节点并且非循环节点
   ...
   */
  addEdge(edgeData) {
    console.log('addEdge==', this.calcPointsList(edgeData))
  ...
  calcPointsList(edgeData) {
    const lf = this.lf
    console.log('calcPointsList=', edgeData)
    console.log(
      'calcPointsList55555=',
      !edgeData.pointsList || edgeData.pointsList.length === 0
    )
  ...
  initView() {
    ...
    this.lf.graphModel.transformModel.translate(
      -rect.minX + GRAPH_START_X,
      -rect.minY + GRAPH_START_Y
    )
  }
